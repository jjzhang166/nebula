...
// 原始出处：https://github.com/qicosmos/Kapok/blob/master/kapok/traits.hpp
// Edit by @benqi
//
// TODO(@benqi): 部分和folly的重合，筛选
//

#ifndef NEBULA_BASE_TRAITS_H_
#define NEBULA_BASE_TRAITS_H_
...
private:
  template<typename U> static auto Check(int) -> decltype(
...
struct is_container : public std::integral_constant<
  bool, detail::has_const_iterator<detail::decay_t<T>>::value &&
        detail::has_begin_end<detail::decay_t<T>>::value &&
        !is_string<T>::value
> {};

template <typename T>
struct is_singlevalue_container : public std::integral_constant<
  bool, !is_std_array<T>::value &&
        !std::is_array<detail::decay_t<T>>::value &&
        !detail::is_tuple<detail::decay_t<T>>::value &&
        is_container<detail::decay_t<T>>::value&&
        !detail::has_mapped_type<detail::decay_t<T>>::value
> {};

template <typename T>
struct is_map_container : public std::integral_constant<
  bool, is_container<detail::decay_t<T>>::value &&
        detail::has_mapped_type<detail::decay_t<T>>::value
> {};

template<typename T>
struct is_normal_class : std::integral_constant<
  bool, std::is_class<detail::decay_t<T>>::value &&
        !is_string<T>::value
> {};

template<typename T>
struct is_basic_type : std::integral_constant<
  bool, std::is_arithmetic<detail::decay_t<T>>::value ||
        is_string<T>::value
> {};
...
template<typename T>
struct is_pointer_ext : std::integral_constant<
  bool, std::is_pointer<detail::decay_t<T>>::value ||
        is_smart_pointer<detail::decay_t<T>>::value
> {};
...
template<typename T>
struct is_container_adapter : std::integral_constant<
  bool, is_queue<T>::value ||
        is_priority_queue<T>::value
> {};

template<typename T>
struct is_user_class : std::integral_constant<
  bool, is_normal_class<T>::value &&
        !is_container_adapter<T>::value &&
        !is_stack<T>::value &&
